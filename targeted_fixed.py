...
CONTENT_PATTERNS = [
    ...
    # username:password format — no $ anchor, handles \r\n
...
def _process_file(conn, share, remote_path, size, unc, findings, args):
    ...
    elif args.dump and not args.scan and size <= MAX_CAT_SIZE:
...
def enumerate_host(args) -> list:
    ...
    # --path overrides share list and sets a start subpath
    start_subpath = ""
    if getattr(args, "path", None):
        _, path_share, start_subpath = parse_unc_path(args.path)
        if path_share:
            target_shares = [path_share]
            info(f"Path mode: share={path_share}, subpath={start_subpath or '(root)'}")
    ...
    for share in target_shares:
        ...
        if start_subpath:
            info(f"Starting at: {start_subpath}")
        print(f"{CYAN}{'─'*60}{RESET}")

        walk_share(conn, share, start_subpath, 0, findings, args)
...
def _write_html_report(findings: list, outfile: str):
    ...
    for i, item in enumerate(findings, 1):
        badge = ""
        if item["patterns"]:
            badge = '<span class="badge">MATCH</span>'
        matches_html = ""
        if item["patterns"]:
...
def parse_args():
    ...
    p.add_argument("--path",
                   help=r"Start path within a share, e.g. \\192.168.1.10\ShareName\SubDir\Files")
    ...
    p.add_argument("-o", "--output",
                   help="Write findings report to this file (.txt or .html)")
